...
typedef unsigned long long GFEventMask;

OF_INLINE GFEventMask GFEventMaskFromType(GFEventType type) {
    return (1 << type);
}

@class GFWindow;

@interface GFEvent : OFObject
{
    double _timestamp;
...
@property (nonatomic, readonly) GFEventType type;
@property (nonatomic, readonly) of_point_t mouseLoaction;
@property (nonatomic, readonly, retain) GFWindow *window;

+ (instancetype)enterExitEventWithType:(GFEventType)type
                             timestamp:(double)timestamp
                                window:(GFWindow *)window;

+ (instancetype)keyEventWithType:(GFEventType)type
                       timestamp:(double)timestamp
                          window:(GFWindow *)window
                        scanCode:(int)scanCode
                       modifiers:(int)mods;

- (instancetype)initWithType:(GFEventType)type
...
- (bool)isEventMatchEventMask:(GFEventMask)mask;
